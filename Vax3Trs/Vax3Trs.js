...
function setup() {
  ...
  endDate.textContent = vaxTbl.getString(vaxTbl.getRowCount() - 1, 0);
...
function drawFrame() {
  ...
  pop();
  fill(0);
  text("3回目接種数 " + vaxTbl.getString(step,0), width/2, height - 4);
  push();
  translate(16, height/2);
  rotate(-PI/2);
  text("累積感染者数 " + vaxTbl.getString(step+DaysDiff,0), 0, 0);
  pop();
}
function draw() {
  drawFrame();
